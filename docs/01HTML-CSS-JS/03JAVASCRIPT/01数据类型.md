...
> join 方法遇到 nullish 会跳过，而加运算会把 null undefined 转为字符串进行字符串拼接
>
...
>
> reduce() 和 reduceRight() 方法 （数组归并） 如果是空数组调用 reduce 方法必须有初始值，否则会报错
>
> 数组遍历方法：
> for...of 不改变原数组
> forEach() 视情况是否改变原数组
...
所以 reduce 如果没有传入初始值，数组长度至少要为 1，且为数组长度为 1 时候，reduce 的回调不会执行，直接返回数组第一项这个初始值
...
3. 转为布尔类型
   ...
   ```js
   Boolean('true') // true
   Boolean('false') // true 所以不可以用Boolean来转换'false'到false!!!!
   Boolean('0') // true
   Boolean('abc') // true
   Boolean('') // false
   Boolean(null) // false
   ...
   假如需要转换 true 到'true'，可以用 JSON.stringify 方法，也可以加空字符串进行转换
   假如需要转换'true'到 true，可以用 JSON.parse 方法 也可以用 `['true',true].includes(xxx)`
